...
const NewsfeedPostListItem = forwardRef<HTMLLIElement, PropTypes>(
  ({ post }, ref) => {
    const [innerPost, setInnerPost] = useState(post);
    const [isEditing, setIsEditing] = useState(false);
    const { comments, onAddCommentSucceeded, onDeleteCommentSucceeded } =
      usePostComments(innerPost._id);
    ...
    return (
      <li
        ref={ref}
        className="p-5 bg-primary mb-10 rounded-md"
      >
        ...
        {post.photo && (
          <img
            className="w-full"
            src={BASE_API_URL + `/posts/${innerPost._id}/photo`}
            alt="avatar"
          />
        )}

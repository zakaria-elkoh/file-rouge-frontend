...
const AllFriendsModal: React.FC<AllFriendsModalProps> = ({ userId }) => {
  ...
  return (
    ...
    >
      <div
        className="rounded-md w-full text-black max-h-[400px] max-w-[500px] bg-primary p-5 relative flex flex-col"
        ref={modalContentRef}
      >
        ...
        <div className="overflow-auto px-2">
          <ul>
            {friends.length === 0 && (
              <p className="my-10 ">You don't have any friends right now!</p>
            )}
            {friends.map((friend) => {
              return (
                <UserListItem
                  onClickListItem={closeModal}
                  key={friend._id}
                  user={friend}
                />
              );
